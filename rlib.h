...
/* -----------------------------------------------------------------------
   ...
   - data:  Contains (len - 12) bytes of payload data for the
            application.fprintf(stderr,"packet.seqno= ");
...
/* -----------------------------------------------------------------------
   ...
   * A rel_t is created by the rel_create function.  The library will call
     rel_create directly for you.
   ...
   * When a packet is received, the library will call either
     rel_recvpkt.  The library already knows what rel_t to use for the
     particular UDP port receiving the packet, and supplies you with the rel_t.
...
/**
 * compute TCP-like checksum
 *
 * @param   data      Pointer to packet over which cksum is computed
 *
 ...
 * @param   len      Length of the packet
 *
 * @return  number of bytes sent if successfull, -1 otherwise
 */
 ...
 * write.
 *
 * @param   buf      Pointer to actual data (not a buffer struct) to be written to output
 *
 * @param   len      number of bytes to be written to output
 **/
 ...
 *
 * @param   buf      Pointer to where data obtained from input should be put
 *
 * @param   len      number of bytes to be read from input
 */
